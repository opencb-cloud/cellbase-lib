...
//		return transList;
	...
	@SuppressWarnings("unchecked")
	public List<FeatureList<Transcript>> getAllByExternalIds(List<String> ids) throws Exception {
		List<FeatureList<Transcript>> featList = getListOfFeatureListByIds(GET_TRANSCRIPTLIST_BY_ID, ids, new BeanArrayListHandler(Transcript.class));
		
		PreparedQuery prepQuery = dBConnector.getDbConnection().createSQLPrepQuery("select e.stable_id, e.chromosome, e.start, e.end, e.strand, t.stable_id, g.stable_id from exon e, exon2transcript et, transcript t, gene g where t.stable_id = ? and t.transcript_id=et.transcript_id and et.exon_id=e.exon_id and t.gene_id=g.gene_id order by e.start");
		FeatureList<Exon> exons = null;
		for(FeatureList<Transcript> transcriptFeautreList: featList) {
			for(Transcript transcript: transcriptFeautreList) {
				prepQuery.setParams(transcript.getId());
				exons = new FeatureList((List<Exon>)prepQuery.execute(new BeanArrayListHandler(Exon.class)));
