...
//			sqlQuery = "select mt.mirna_id from xref x, transcript2xref tx, mirna_target mt, transcript t, gene g where x.display_id= ? and x.xref_id=tx.xref_id and tx.transcript_id= mt.transcript_id and tx.transcript_id=t.transcript_id and t.gene_id=g.gene_id and (select count(distinct(tn.gene_id)) from mirna_target mtn, transcript tn where mtn.mirna_id=mt.mirna_id and mtn.transcript_id=tn.transcript_id) between " + mirnaTargetFilter.getMinNumberGenes() + " and " + mirnaTargetFilter.getMaxNumberGenes() + " group by mt.mirna_id;";
			sqlQuery = "select mt.mirna_id from xref x, transcript2xref tx, mirna_target mt, annotation_term_gene_count at  where x.display_id= ? and x.xref_id=tx.xref_id and tx.transcript_id= mt.transcript_id and mt.mirna_id=at.term and at.number_genes between " + mirnaTargetFilter.getMinNumberGenes() + " and " + mirnaTargetFilter.getMaxNumberGenes() + " group by mt.mirna_id;";
		}else {
...
//			sqlQuery = "select jt.tf_factor_name from jaspar_tfbs jt, xref x, transcript2xref tx, transcript t where  x.display_id = ? and x.xref_id=tx.xref_id and tx.transcript_id=t.transcript_id and t.gene_id=jt.gene_id and (select count(distinct(gene_id)) from jaspar_tfbs jtn where jt.tf_factor_name=jtn.tf_factor_name) between " + jasparFilter.getMinNumberGenes() + " and " + jasparFilter.getMaxNumberGenes() + " group by jt.tf_factor_name";
			sqlQuery = "select jt.tf_factor_name from jaspar_tfbs jt, xref x, transcript2xref tx, transcript t, annotation_term_gene_count at where  x.display_id = ? and x.xref_id=tx.xref_id and tx.transcript_id=t.transcript_id and t.gene_id=jt.gene_id and jt.tf_factor_name=at.term and at.number_genes between " + jasparFilter.getMinNumberGenes() + " and " + jasparFilter.getMaxNumberGenes() + " group by jt.tf_factor_name";
		}else {
