...
class BioPaxHibernateDBAdaptor extends HibernateDBAdaptor implements BioPaxDBAdaptor {
	...
	public BioPaxHibernateDBAdaptor(SessionFactory sessionFactory) {
		super(sessionFactory);
	}

	public BioPaxHibernateDBAdaptor(SessionFactory sessionFactory, String species, String version) {
	...
	private static String COMPLEX_QUERY_BY_ID = "select * from Complex c where PK_Complex = ";
	...
	public Dna getDna(PhysicalEntity ph) {
		...
		return (Dna) this.openSession().get(Dna.class, id);
	}
	...
	public Dnaregion getDnaregion(PhysicalEntity ph) {
		...
		return (Dnaregion) this.openSession().get(Dnaregion.class, id);
	}
	...
	public Rna getRna(PhysicalEntity ph) {
		...
		return (Rna) this.openSession().get(Rna.class, id);
	}
	...
	public Rnaregion getRnaregion(PhysicalEntity ph) {
		...
		return (Rnaregion) this.openSession().get(Rnaregion.class, id);
	}
	...
	public SmallMolecule getSmallMolecule(PhysicalEntity ph) {
		...
		return (SmallMolecule) this.openSession().get(SmallMolecule.class, id);
	}
	...
	public Complex getComplex(PhysicalEntity ph) {
		...
		return (Complex) this.openSession().get(Complex.class, id);
	}

	public Complex getComplex(int complexId) {
		String sql = COMPLEX_QUERY_BY_ID + complexId;
		Query query = this.openSession().createSQLQuery(sql).addEntity(Complex.class);
	...
	public Complex getComplex(String complexName, String dataSourceName) {
		Complex output = null;
		try {
			String sql = "select c.* from Complex c, PhysicalEntity ph, nameEntity nm, DataSource ds, entity_dataSource eds where nm.nameEntity = '" + complexName + "' and c.physicalentity = ph.PK_PhysicalEntity and  ph.entity = nm.entity and eds.datasource = ds.PK_DataSource and eds.entity = ph.entity and ds.sourceName = '" + dataSourceName + "'";
			Query query = this.openSession().createSQLQuery(sql).addEntity(Complex.class);
	...
	public List<ComplexComponent> getComplexComponents(Complex input) {
		...
		Iterator<PhysicalEntity> it = input.getPhysicalEntities().iterator();
		while (it.hasNext()) {
...
//		Protein protein = null;
		...
		return (Protein) this.openSession().get(Protein.class, proteinId);
	}	

	public Protein getProtein(int proteinId) {
		String sql = PROTEIN_QUERY_BY_ID + proteinId;
		Query query = this.openSession().createSQLQuery(sql).addEntity(Protein.class);
	...
	public Catalysis getCatalysis(Control c) {
		...
		return (Catalysis) this.openSession().get(Catalysis.class, id);
	}
	...
	public Control getControl(Interaction ph) {
		...
		return (Control) this.openSession().get(Control.class, id);
	}
	...
	public Conversion getConversion(Interaction ph) {
		...
		return (Conversion) this.openSession().get(Conversion.class, id);
	}
	...
	public GeneticInteraction getGeneticInteraction(Interaction ph) {
		...
		return (GeneticInteraction) this.openSession().get(GeneticInteraction.class, id);
	}
	...
	public MolecularInteraction getMolecularInteraction(Interaction ph) {
		...
		return (MolecularInteraction) this.openSession().get(MolecularInteraction.class, id);
	}
	...
	public TemplateReaction getTemplateReaction(Interaction ph) {
		...
		return (TemplateReaction) this.openSession().get(TemplateReaction.class, id);
	}
	...
	public List<Interaction> getInteractions(PhysicalEntity input) {
		...
		Query query = this.openSession().createSQLQuery(sql).addEntity(Interaction.class);
		...
		query = this.openSession().createSQLQuery(sql).addEntity(Interaction.class);
		aux = (List<Interaction>) query.list();
		output.addAll(aux);
	...
	public List<Pathway> getPathways(String dataSourceName, String search, boolean onlyTopLevel) {
		...
		try {
			...
			if (onlyTopLevel) {
				sql = "select distinct p.* from pathway_pathway_pathwayComponent pc, Pathway p, entity_dataSource eds, DataSource ds, nameEntity ne where " + searchSql + " pc.pathway = p.PK_Pathway and p.entity = eds.entity and eds.datasource = ds.PK_DataSource and p.entity=ne.entity and ds.sourceName = '" + dataSourceName + "' and pc.pathway not in (select pathwayComponent from pathway_pathway_pathwayComponent)";				
			} else {
				sql = "select pw.* from Pathway pw, DataSource ds, entity_dataSource eds where  " + searchSql + " eds.datasource = ds.PK_DataSource and eds.entity = pw.entity and ds.sourceName = '" + dataSourceName + "'"; //, nameEntity ne
			}
			System.out.println("getPathways(): "+sql);
			Query query = this.openSession().createSQLQuery(sql).addEntity(Pathway.class);
	...
	public List<DataSource> getDataSources() {
		...
		try {
			...
			Query query = this.openSession().createSQLQuery(sql).addEntity(DataSource.class);
