...
public class MiRnaGeneDBManager extends DBManager {
	...
	public final static String GET_ALL_BY_POSITION = "select mg.* from transcript t, transcript2xref tx, xref x, dbname db, mirna_gene mg where t.transcript_id=tx.transcript_id and tx.xref_id=x.xref_id and x.dbname_id=db.dbname_id and db.dbname='mirna_gene' and x.display_id=mg.id ";
	...
	public FeatureList<MiRnaGene> getAllByPosition(String chromosome, int position) throws SQLException, IllegalAccessException, ClassNotFoundException, InstantiationException {
		...
		return getFeatureList(GET_ALL_BY_POSITION+" and t.chromosome = '"+chromosome+"' and t.start <= "+position +" and t.end >= " + position, new BeanArrayListHandler(MiRnaGene.class));
	}
