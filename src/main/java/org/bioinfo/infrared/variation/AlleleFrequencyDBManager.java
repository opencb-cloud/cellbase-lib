...
import org.bioinfo.infrared.core.variation.AlleleFrequency;
...
public class AlleleFrequencyDBManager extends DBManager {
	...
	public void writeAllByMAF(double min, double max, String population, String outfile) throws SQLException, IllegalAccessException, ClassNotFoundException, InstantiationException, IOException {
		if(min > max) {
			double swap = max;
			max = min;
			min = swap;
		}
		writeFeatureList("SELECT "+SELECT_FIELDS+" FROM allele_frequency af, snp s where af.other_allele_freq>="+min+" and af.other_allele_freq<="+max+" and af.population='"+population+"' and af.snp_id=s.snp_id", new File(outfile));
	}
